...
%             curTransition = 0.005;
            ctrlPoint = curTransition+(0:0.0005:0.002);
            closeAngle = deg2rad(par.closeAngle);
            areaAtTrans = 2*pi.*(curTransition.^-1).^2.*(1-cos(closeAngle+pi./2));
            areaAtTransCP = 2*pi.*(ctrlPoint.^-1).^2.*(1-cos(ctrlPoint+pi./2));
            ctrlPoint = [-ctrlPoint 0 ctrlPoint];
            areaAtTransCP = [areaAtTransCP areaAtTrans*3 areaAtTransCP];
            pp = spline(ctrlPoint, areaAtTransCP);
            
            
            f1 = @(x)ppval(pp,x);    % the parabola around 0 curvature
