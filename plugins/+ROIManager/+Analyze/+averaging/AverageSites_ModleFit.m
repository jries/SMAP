classdef AverageSites_ModleFit<interfaces.DialogProcessor&interfaces.SEProcessor
    methods
        function obj=AverageSites_ModleFit(varargin)        
    ...
    %             for k=1:length(sites) % later to put options here
                    if p.sortselection.Value==1 ||  sites(k).annotation.use
                        ...
                        [locsSite,indloc] = dcal.getLocs({'xnmrot','ynmrot'},'size',roisize','grouping','ungrouped'); % per ROI info.
                        indloc = find(indloc); 
