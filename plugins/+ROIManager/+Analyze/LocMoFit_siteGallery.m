classdef LocMoFit_siteGallery<interfaces.DialogProcessor&interfaces.SEProcessor
    properties
        fig = gobjects(1);
    end
    methods
        function obj=LocMoFit_siteGallery(varargin)        
...
%             p.fitterGUI_name = 'LocMoFitGUI';
            eval = obj.locData.SE.processors.eval.guihandles.modules.Data(:,2);
            idxFitterGUI = strcmp(p.fitterGUI_name.selection,eval);
            ...
            idx = strcmp(fn, p.fitterGUI_name.selection);
            ...
            for np = 1:nPage
                ...
                for k = 1:nSiteThisPage
                    ...
                    fitter.allParsArg = subSites(siteInd).evaluation.(p.fitterGUI_name.selection).allParsArg;
...
pard.useOrder.object=struct('String','order','Style','checkbox', 'value',0);
pard.useOrder.position=[rowRun,1.3];
pard.useOrder.Width=1;
...
pard.t_fitterGUI_name.object=struct('String','Which LocMoFit','Style','text');
pard.t_fitterGUI_name.position=[rowRun+3,3];
pard.t_fitterGUI_name.Width=1;

evalPlugins = fieldnames(obj.SE.processors.eval.children);
options = evalPlugins(startsWith(evalPlugins, 'LocMoFitGUI'));
options = strjoin(options,'|');
pard.fitterGUI_name.object=struct('String',options,'Value',1,'Style','popupmenu');
pard.fitterGUI_name.position=[rowRun+3,col4];
pard.fitterGUI_name.Width=1;
...
pard.outline.object=struct('String','Model outline','Style','checkbox','value',1);
pard.outline.position=[rowUpdate+5,1];
pard.outline.Width=1;
