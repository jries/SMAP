classdef Cluster_MINFLUX<interfaces.DialogProcessor
    ...
    methods
        ...
        function out=run(obj,p)      
            locs=obj.locData.getloc({'xnm','ynm','time','numberInGroup','filenumber','efo','cfr'},'layer',find(obj.getPar('sr_layerson')),'Position','roi');
            ...
            header=sprintf('nlocs \t on-time \t dtmin \t dtmedian \t dtmean \t sigmax \t sigmay \t sigmaxrobust \t sigmayrobust \t sigmaxdetrend \t sigmaydetrend \t efo \t cfr \t filename' );
            disp(header)
            results=sprintf([num2str(nlocs)  '\t' num2str(ontime)  '\t' num2str(dtmin)  '\t' num2str(dtmedian) '\t' num2str(dtmean) '\t' num2str(sigmax) ...
                 '\t' num2str(sigmay)  '\t' num2str(sxrobust)  '\t' num2str(syrobust)  '\t' num2str(sxdetrend)  '\t' num2str(sydetrend) '\t' num2str(efo) '\t' num2str(cfr) '\t' filename]  );
