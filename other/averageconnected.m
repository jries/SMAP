            ...
            for ny=-1:1
                ...
                if n(x+nx, y+ny) > n(x,y)
                    if (dxmap(x+nx, y+ny)-dxmap(x, y))^2+(dymap(x+nx, y+ny)-dymap(x, y))^2<dmaxlat^2 && (dzmap(x+nx, y+ny)-dzmap(x, y))^2<dmaxax^2
                        ...
                        dxmapc(x+nx,y+ny)=dxmapc(x+nx,y+ny)+dxmap(x,y); %add map x,y to map x+xn,y+yn %here: add weighting!
...
%                         normcounter(x+nx,y+ny)=normcounter(x+nx,y+ny)+1;
                    ...
                    if (dxmap(x+nx, y+ny)-dxmap(x, y))^2+(dymap(x+nx, y+ny)-dymap(x, y))^2<dmaxlat^2 && (dzmap(x+nx, y+ny)-dzmap(x, y))^2<dmaxax^2
                        dxmapc(x,y)=dxmapc(x,y)+dxmap(x+nx,y+ny);
...
dxmapfinal=dxmapc./weights;
  figure(88);
imagesc(vertcat(horzcat(nstart,n,normcounter),horzcat(dxmap,dxmapc,dxmapfinal)))
