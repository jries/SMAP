...
//********************************************************************************************************
//global fit GaussFit sigma
__global__ void kernel_MLEFit_sigma_sCMOS_multi(const float* d_data, const float PSFSigma, const float* d_dTAll, const int sz, const int iterations, const int NV, const int noChannels, float* d_Parameters, float* d_CRLBs, float* d_LogLikelihood, const int Nfits, const int* d_shared, const float* d_varim) {
	//float test = 0;
	...
	for (i = 0; i < noChannels; i++) {
		s_varim[i] = d_varim + (sz * sz * bx * BlockSize + sz * sz * tx + sz * sz * Nfits * i);
	}


	dT = d_dTAll + (bx * BlockSize + tx) * 5 * noChannels * 2;
	cshared = d_shared + (bx * BlockSize + tx) * 5;
	...
	for (kk = 0; kk < NV; kk++) d_Parameters[Nfits * kk + BlockSize * bx + tx] = newThetaAll[kk];
...
__global__ void kernel_splineMLEFit_z_sCMOS_multi(const float* d_data, const float* d_coeff, const float* d_dTAll, const int spline_xsize, const int spline_ysize, const int spline_zsize, const int sz, const int iterations,
	...
	dT = d_dTAll + (bx * BlockSize + tx) * 5 * noChannels * 2;
	cshared = d_shared + (bx * BlockSize + tx) * 5;
	float initZ = d_initZall[bx * BlockSize + tx];
	...
	for (i = 0; i < noChannels; i++) {
		...
		//maxJump_Init[3+5*i] = max(maxJump_Init[3+5*i],newTheta[3+5*i]/dT[noChannels * 5 + 3 + i * 5]);

		//maxJump_Init[4+5*i] = max(maxJump_Init[4+5*i],newTheta[4+5*i] / dT[noChannels * 5 + 4 + i * 5]);
	}
	...
	for (i = 0; i < 5; i++) {
		...
		else
		{
			for (j = 0; j < noChannels; j++) {
				maxJump[n + j] = maxJump_Init[i + j * 5];
				newThetaAll[n + j] = newTheta[i + j * 5];
				//newThetaAll[n+j]=newTheta[i+j*5]/ dT[noChannels * 5 + i + j * 5]- dT[i + j * 5];
			}
			n = n + j - 1;
		}
		n = n + 1;
	...
	memset(hessian, 0, NV * NV * sizeof(float));
	//memset(newDudtAll,0,NV*noChannels*sizeof(float));
	memset(newDudtAll, 0, NV * noChannels * noChannels * sizeof(float));
	...
	for (ii = 0; ii < sz; ii++) for (jj = 0; jj < sz; jj++) {
		...
		for (i = 0; i < 5; i++) {
			...
			else
			{
				for (j = 0; j < noChannels; j++) {
					newDudtAll[n + j + j * NV] = newDudt[i + j * 5];
					//newDudtAll[n+j+j*NV]=newDudt[i+j*5] * dT[noChannels * 5 + i + j * 5];
				}
				n = n + j - 1;
			}
			n++;
				...
				for (i = 0; i < 5; i++) {
					...
					else
					{
						for (j = 0; j < noChannels; j++) {
							...
							newTheta[i + 5 * j] = newThetaAll[n + j] + dT[i + j * 5];
							//newTheta[i + 5 * j] = newThetaAll[n + j] * dT[noChannels * 5 + i + j * 5] + dT[i + j * 5];
							//newTheta[i + 5 * j] = newThetaAll[n] * dT[noChannels * 5 + i + j * 5] + dT[i + j * 5];
