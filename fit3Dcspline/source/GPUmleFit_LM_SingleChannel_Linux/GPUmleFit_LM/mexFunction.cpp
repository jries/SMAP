...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
	...
	*/
    ...
    struct timeval tpstart,tpend,tpfreq;
...
if  (silent==0)
{
	...
	togpu=(tpend.tv_sec-tpstart.tv_sec)+tpend.tv_usec-tpstart.tv_usec;
	gettimeofday(&tpstart,NULL);
	#endif
...
if  (silent==0)
   ...
   //             dim3 dimGrid(Nfitraw);
	...
	#elif defined(LINUX_IMPL)
	gettimeofday(&tpend,NULL);
	fit=(tpend.tv_sec-tpstart.tv_sec)+tpend.tv_usec-tpstart.tv_usec;
	gettimeofday(&tpstart,NULL);
	#endif
	...
	#elif defined(LINUX_IMPL)
	gettimeofday(&tpend,NULL);
	fromgpu =(tpend.tv_sec-tpstart.tv_sec)+tpend.tv_usec-tpstart.tv_usec;
	gettimeofday(&tpstart,NULL);
	#endif
	...
	#ifdef WINDOWS_IMPL
	QueryPerformanceCounter(&stop);    
	cleanup = (double)(stop.QuadPart-start.QuadPart)/freq.QuadPart;
	#elif defined(LINUX_IMPL)
	gettimeofday(&tpend,NULL);
	cleanup=(tpend.tv_sec-tpstart.tv_sec)+tpend.tv_usec-tpstart.tv_usec;	
	#endif
